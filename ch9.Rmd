...
exchange.sort.asc <- function(input_vector) {
  ...
  vector_length <- length(input_vector_clone)
  for (i in 1:(vector_length - 1)) {
    for (j in (i + 1):vector_length) {
      if (input_vector_clone[i] > input_vector_clone[j]) {
        temp <- input_vector_clone[i]
        input_vector_clone[i] <- input_vector_clone[j]
        input_vector_clone[j] <- temp
      }
    }
  }
...
- 將課堂中的自訂排序函數加入 `reverse = ` 的參數讓使用者可以指定遞增或遞減排序
- 自訂計算[樣本標準差](https://zh.wikipedia.org/wiki/%E6%A8%99%E6%BA%96%E5%B7%AE)的函數並上傳至 GitHub